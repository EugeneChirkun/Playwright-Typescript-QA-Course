...
После этого модуля студент сможет подготовить рабочее окружение Automation QA, установить и проверить Node.js и npm, создать первый Playwright project, запустить тесты, открыть HTML report и добавить базовые npm scripts для ежедневной работы.

## Почему это важно для Automation QA

Automation QA — это не только написание тестов. Инженер также работает с инструментами, структурой проекта, зависимостями, командами запуска, настройками test runner и отчетами. Если окружение настроено неправильно, тесты могут не запускаться, вести себя нестабильно или давать непонятные ошибки.
...
- VS Code как основной редактор для automation-проекта.
- Установка Playwright через terminal и VS Code extension.
...
Рабочее окружение Automation QA — это набор инструментов, которые нужны, чтобы писать, запускать и поддерживать автоматизированные тесты. Обычно в него входят runtime, менеджер пакетов, редактор кода, test runner, browser binaries, terminal, Git и инструменты для отчетов.

Для курса нам нужны Node.js, npm, VS Code и Playwright. Эти инструменты позволяют создать тестовый проект, установить зависимости, запускать тесты из terminal и анализировать результаты через HTML report.
...
Playwright использует Node.js, потому что сам test runner, CLI-команды, конфигурация и npm packages работают в Node.js-окружении. Для обучения и реальных проектов лучше использовать LTS-версию Node.js: она стабильнее и дольше поддерживается.
...
Если обе команды показывают версии, значит Node.js и npm доступны в terminal.
...
- `devDependencies` — packages, которые нужны для разработки, тестирования и сборки. Playwright обычно находится здесь, потому что нужен для тестов.
...
- `node_modules` — папка, куда npm устанавливает packages. Ее не коммитят в Git, потому что она большая и может быть восстановлена через `npm install` или `npm ci`.
...
- `npx` — запускает CLI-команду из установленного package без ручного поиска пути к файлу.
...
VS Code — удобный редактор для Automation QA. В нем можно писать тесты, открывать структуру проекта, искать по файлам, запускать команды во встроенном terminal и читать ошибки прямо рядом с кодом.

Полезные возможности VS Code:

- File Explorer помогает быстро ориентироваться в структуре проекта.
- Search помогает находить tests, locators, fixtures и настройки.
- Встроенный terminal позволяет запускать `npm`, `npx` и Playwright commands без отдельного окна.
...
- Установить browsers — выбираем `yes`, чтобы Playwright сразу скачал browser binaries.
...
После установки Playwright project обычно содержит:
...
- `playwright.config.ts` — настройки Playwright test runner: browsers, timeout, retries, reporter и другие параметры.
- `package.json` — зависимости и npm scripts проекта.
- `package-lock.json` — точные версии dependencies.
- `node_modules/` — установленные packages; эту папку не нужно коммитить.
...
- `npx playwright test` запускает все тесты в headless mode.
- `npx playwright test --headed` запускает тесты с видимым browser window.
...
- `import { test, expect } from '@playwright/test';` подключает функции Playwright test runner.
- `test(...)` объявляет тест и его название.
- `async` показывает, что внутри теста будут асинхронные действия.
- `page` — объект browser page, через который тест открывает страницы и взаимодействует с UI.
...
Scripts полезны, потому что команда становится короче, понятнее и одинаковой для всей команды.

## Практика на занятии

1. Проверить версии Node.js и npm.
2. Создать отдельный Playwright project для практики.
...
12. Как запустить тесты Playwright из terminal?
...
В этом модуле студент настраивает базовое окружение Automation QA, понимает роль Node.js, npm, VS Code и Playwright, создает первый Playwright project, запускает тесты, открывает HTML report и добавляет npm scripts для удобной ежедневной работы.
