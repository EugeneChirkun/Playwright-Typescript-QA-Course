...
- типизированные данные для QA-сценариев;
- `type`, `interface` и union types;
- разделение кода с помощью `export` и `import`.

## Минимальный результат

После модуля студент должен уметь:

- описать пользователя и тест-кейс типами;
- написать типизированную вспомогательную функцию;
- организовать данные и импорты по нескольким файлам.
...
TypeScript помогает находить такие ошибки до запуска тестов. Типизированные helper functions проще переиспользовать, а понятные типизированные данные легче читать на ревью кода в PR.
...
Для практики модуля 04 TypeScript-код нужно размещать в отдельном репозиторий домашних заданий в папке:
