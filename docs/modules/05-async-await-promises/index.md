...
- состояния и назначение `Promise`;
- последовательное выполнение с `async` и `await`;
- ошибки и безопасный параллельный запуск независимых операций.

## Минимальный результат

После модуля студент должен уметь:

- объяснить, что возвращает асинхронная функция;
- правильно поставить `await`;
- выбрать между последовательным выполнением и `Promise.all`.
